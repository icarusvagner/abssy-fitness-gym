  ...
  <q-page padding>
    ...
    >
      ...
      <template v-slot:body="props">
        <q-tr :props="props" @click="onRowClick(props.row)" :class="props.row.status === 'removed' ? 'cursor-pointer bg-red-5 text-white' : 'cursor-pointer'">
    ...
    <q-card style="width: 700px; max-width: 80vw;" class="column q-gutter-y-md q-pa-sm">
      ...
      >
        ...
        <q-card-section class="column q-gutter-y-sm">
          ...
          <q-input filled v-model="form.duration" type="number" label="Duration" min="1" :rules="[ val => val > 0 || 'Please enter duration of package' ]" />
          ...
          <q-input filled v-model="form.price" type="number" label="Price" min="1" :rules="[ val => val > 0 || 'Please enter package price' ]" />
          <q-select label="Benefits" filled v-model="benefits" use-input use-chips multiple hide-dropdown-icon input-debounce="0" new-value-mode="add-unique" class="col" :rules="[ val => !!val || 'Enter benefits' ]" />
        </q-card-section>
    ...
    <q-card style="width: 700px; max-width: 80vw;" class="q-pa-sm">
      ...
      >
        ...
        <q-card-section class="q-pt-none">
          ...
          <q-select label="Benefits" filled v-model="benefits" use-input use-chips multiple hide-dropdown-icon input-debounce="0" new-value-mode="add-unique" class="col" :rules="[ val => !!val || 'Enter benefits' ]" />
          <q-select filled v-model="formUpdate.pack_status" :options="options2" label="Package avaibility" />
        </q-card-section>


        <q-card-actions align="right">
          <q-btn flat label="Close" color="primary" v-close-popup />
          <q-btn :disabled="isUpdate" color="secondary" label="Save changes" type="submit" :loading="isLoading" @click="isLoading = !isLoading" />
        </q-card-actions>
      </q-form>
    </q-card>
  </q-dialog>
...
<script setup lang="ts">
  ...
  const options = ref(['week','weeks','month','months','year','years']);
