...
use crate::{config::core_config, middleware::TokenClaims};

pub fn extractor_token(token: &str) -> Result<TokenClaims> {
    let _key: Hmac<Sha256> = Hmac::new_from_slice(&core_config().JWT_SECRET.as_bytes()).unwrap();
    let decoded_key = DecodingKey::from_secret(&core_config().JWT_SECRET.as_bytes());
